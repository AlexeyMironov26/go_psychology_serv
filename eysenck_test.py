...
def interpret_eysenck(scores: dict) -> str:
    ...
    # Определяем экстраверсию
    if e >= 11:
        e_type = "экстраверт"
    else:
        e_type = "интроверт"
    
    # Определяем нейротизм 
    if n >= 11:
        n_type = "высокий нейротизм"
    else:
        n_type = "эмоциональная устойчивость"
    
    # Тип темперамента
    if e_type == "экстраверт" and n_type == "эмоциональная устойчивость":
        result = """Ваш тип темперамента - Сангвиник
Вы - человек с устойчивой психикой, открытый миру и общительный.
Вы часто становитесь душой компании, имеете выраженные лидерские качества 
и быстро адаптируетесь к изменениям. Однако Вам может быть сложно
долго заниматься одним и тем же, доводить начатое до конца, переживать скуку. 
Рекомендуем Вам развивать свои лидерские качества, а также добавить в жизнь «структуру» - 
ставить цели, прописывать для себя конкретные шаги и использовать техники тайм-менеджмента. 
"""
    
    elif e_type == "экстраверт" and n_type == "высокий нейротизм":
        result = """Ваш тип темперамента - Холерик
Вы - энергичный и увлекающийся человек. Быстро включаетесь в новые дела, инициативны
и целеустремленны. Однако присущая Вам эмоциональность и импульсивность может мешать Вам
качественно выполнять свою работу и выстраивать крепкие отношения. 
Рекомендуем Вам учиться управлять своими эмоциями, найти адаптивные формы их выражения и переживания. 
Для этого может помочь занятия спортом, творчеством, психологией. 
"""
    
    elif e_type == "интроверт" and n_type == "эмоциональная устойчивость":
        result = """Ваш тип темперамента - Флегматик
Вы - спокойный и надёжный человек.  Вас сложно вывести из равновесия даже в стрессовых ситуациях.
Вы предпочитаете стабильность, сдержанность и действуете размеренно и обдуманно.
Однако излишняя эмоциональная сдержанность может считываться окружающими Вас людьми как «холод» 
и мешать выстраивать крепкие отношения, а желание избегать конфликтов приводить к психологическому
дискомфорту и накоплению негативных эмоций. 
Рекомендуем Вам развивать личные границы и умения их отстаивать, а также развивать 
эмоциональный интеллект, чтобы лучше выражать свои чувства и понимать эмоции других людей. 
"""
    
    elif e_type == "интроверт" and n_type == "высокий нейротизм":  
        result = """Ваш тип темперамента - Меланхолик
Вы - тонко чувствующий и глубокий человек. Вам характерны повышенная чувствительность, ранимость, 
тревожность и склонность к постоянному самоанализу. При этом Вы очень эмпатичны и обладаете богатым 
внутренним миром, что делает Вас очень внимательным к другим людям. Однако застревание в негативных 
эмоциях и низкая стрессоустойчивость могут мешать Вам выстраивать крепкие взаимоотношения и проявлять 
себя в различных сферах.
Рекомендуем Вам учиться совладать с негативными эмоциями, развивать навыки стрессоустойчивости 
...
"""
    ...
    if 0<=e<=4 or 20<=e<=24 or 20<=n<=24:
        result += "\n\nРекомендуем Вам обратиться к психологу"
