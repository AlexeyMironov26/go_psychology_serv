...
Вам легко заводить новые знакомства, вы часто становитесь душой компании.
Вы жизнерадостны, оптимистичны, быстро адаптируетесь к изменениям. 
Вам нравится быть в центре внимания, вы любите лидерство и активное общение.
Ваше настроение стабильно, стресс переносите легко."""
    
    elif e_type == "экстраверт" and n_type == "высокий нейротизм":
        result = """Ваш тип темперамента - Холерик
Вы — энергичный и увлекающийся человек. 
Быстро включаетесь в новые дела, активно выражаете свои эмоции.
Вы импульсивны, но это делает вас инициативным и целеустремлённым. 
Ваше настроение может меняться в зависимости от ситуации.
Вам важно научиться управлять своим темпом и находить баланс между активностью и отдыхом."""
    
    elif e_type == "интроверт" and n_type == "эмоциональная устойчивость":
        result = """Ваш тип темперамента - Флегматик
Вы — спокойный и надёжный человек. 
Вас сложно вывести из равновесия, вы действуете размеренно и обдуманно.
Вы предпочитаете стабильность, не склонны к импульсивным поступкам. 
В общении вы сдержанны, но верны друзьям и надёжны в отношениях.
Вас ценят за основательность, терпение и умение сохранять спокойствие в сложных ситуациях."""
    
    elif e_type == "интроверт" and n_type == "высокий нейротизм":  
        result = """Ваш тип темперамента - Меланхолик
Вы — тонко чувствующий и глубокий человек. Вы склонны к размышлениям и самоанализу.
Внешне вы можете казаться сдержанным, но внутри переживаете очень ярко. 
Вы впечатлительны, эмпатичны, обладаете богатым внутренним миром.
Ваша чувствительность — это ваша сила, она позволяет вам замечать то, что другие упускают. 
Вам важно научиться заботиться о своём эмоциональном состоянии."""
    else: 
